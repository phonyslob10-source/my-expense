...
export async function getTransactions(params: URLSearchParams = new URLSearchParams()) {
  return request<Transaction[]>(`/transactions?${params.toString()}`)
}
export async function createTransaction(tx: Partial<Transaction>) {
  return request<Transaction>('/transactions', { method: 'POST', body: JSON.stringify(tx) })
}
export async function updateTransaction(id: string, tx: Partial<Transaction>) {
  return request<Transaction>(`/transactions/${id}`, { method: 'PUT', body: JSON.stringify({ ...tx, id }) })
}
export async function deleteTransaction(id: string) {
  return request<Transaction>(`/transactions/${id}`, { method: 'DELETE' })
}
...
export async function loginAdmin(password: string) {
  const result = await request<{ access_token: string }>('/admin/login', { method: 'POST', body: JSON.stringify({ password }) })
...
export async function syncTransactions(changes: Transaction[]) {
  return request<{
    server_time: string
    ...
    categories: Category[]
    members: Member[]
  }>('/sync', { method: 'POST', body: JSON.stringify({ device_id: deviceId(), changes }) })
}
