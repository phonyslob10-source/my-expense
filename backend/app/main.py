...
def get_transactions(start: Optional[str] = None, end: Optional[str] = None, type: Optional[str] = None,
                     category: Optional[str] = None, member: Optional[str] = None, q: Optional[str] = None):
    clauses = ["is_deleted=0"]; args: list = []
...
def update_transaction(tx_id: str, payload: TransactionIn, x_device_id: Optional[str] = Header(default=None), authorization: Optional[str] = Header(default=None)):
    with get_conn() as conn:
        ...
        is_admin = False
        if authorization:
            try: require_admin(authorization); is_admin = True
            except HTTPException: pass
        if not is_admin and existing["device_id"] != (x_device_id or ""):
            raise HTTPException(403, "只能修改自己设备创建的记录")
        payload.id = tx_id; payload.device_id = existing["device_id"]
...
def delete_transaction(tx_id: str, x_device_id: Optional[str] = Header(default=None), authorization: Optional[str] = Header(default=None)):
    with get_conn() as conn:
        ...
        now = utc_now(); conn.execute("UPDATE transactions SET is_deleted=1, updated_at=? WHERE id=?", (now, tx_id))
...
def summary(period: str = Query("month"), date_value: str = Query(..., alias="date"), start: Optional[str] = None, end: Optional[str] = None):
    if period == "custom":
        ...
        s = f"{date_value[:7]}-01"; d = datetime.strptime(s, "%Y-%m-%d").date(); next_month = (d.replace(day=28)+timedelta(days=4)).replace(day=1); e = (next_month-timedelta(days=1)).isoformat()
    with get_conn() as conn:
    ...
    for r in rows:
        ...
        if typ == "expense":
            k=r["category1"] or "未分类"; by_category1[k]=by_category1.get(k,0)+amt
            k=r["category2"] or "未分类"; by_category2[k]=by_category2.get(k,0)+amt
        else:
            k=r["category1"] or "未分类"; by_income_category[k]=by_income_category.get(k,0)+amt
        m=r["member"]; by_member.setdefault(m,{"expense":0,"income":0})[typ]+=amt
        bucket=r["date"] if period in ("day","custom","week") else r["date"][:7]
...
def add_member(body: MemberIn, _=Depends(require_admin)):
    with get_conn() as conn:
        ...
        mid=body.id or str(uuid.uuid4()); now=utc_now(); conn.execute("INSERT INTO members(id,name,sort_order,is_active,updated_at) VALUES (?,?,?,?,?)",(mid,body.name,body.sort_order,1,now))
...
def update_member(mid: str, body: MemberIn, _=Depends(require_admin)):
    with get_conn() as conn:
        ...
        now=utc_now(); conn.execute("UPDATE members SET name=?,sort_order=?,is_active=?,updated_at=? WHERE id=?",(body.name,body.sort_order,int(body.is_active),now,mid)); return row_member(conn.execute("SELECT * FROM members WHERE id=?",(mid,)).fetchone())
...
def sync(body: SyncRequest):
    with get_conn() as conn:
        accepted=[upsert_transaction(conn,tx,body.device_id) for tx in body.changes]
...
def infer_type(conn, category1: str) -> str:
    row=conn.execute("SELECT type FROM categories WHERE is_active=1 AND name=? ORDER BY CASE type WHEN 'income_category' THEN 0 ELSE 1 END LIMIT 1",(category1,)).fetchone()
    return "income" if row and row["type"]=="income_category" else "expense"


def import_rows(text: str, conn) -> tuple[int,list[str]]:
    text=text.lstrip("\ufeff")
    reader=csv.DictReader(io.StringIO(text))
    if not reader.fieldnames: raise HTTPException(400,"CSV 文件为空")
    ...
    for line_no,row in enumerate(reader,start=2):
        ...
        project=clean_import_value(get("项目" if is_cn else "project")); member=clean_import_value(get("账户" if is_cn else "member")); category1=clean_import_value(get("分类" if is_cn else "category1")); category2=clean_import_value(get("分类2" if is_cn else "category2")); dt=parse_import_date(get("日期" if is_cn else "date"))
        if not project or not member or not dt:
            errors.append(f"第{line_no}行：项目、账户或日期无效"); continue
        try: amount=parse_amount(get("金额" if is_cn else "amount"))
        except Exception: errors.append(f"第{line_no}行：金额无效"); continue
        typ=get("type") if not is_cn else infer_type(conn,category1)
        if typ not in ("expense","income"): typ=infer_type(conn,category1)
        txid=clean_import_value(get("id")) or str(uuid.uuid4()); now=utc_now()
        conn.execute("INSERT INTO transactions(id,project,date,member,type,category1,category2,amount_cents,note,created_at,updated_at,device_id,is_deleted) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,0) ON CONFLICT(id) DO UPDATE SET project=excluded.project,date=excluded.date,member=excluded.member,type=excluded.type,category1=excluded.category1,category2=excluded.category2,amount_cents=excluded.amount_cents,note=excluded.note,updated_at=excluded.updated_at",(txid,project,dt,member,typ,category1 or None,category2 or None,amount,clean_import_value(get("note")),clean_import_value(get("created_at")) or now,now,"import"))
        count+=1
    return count,errors
...
def iter_export_rows(rows):
    ...
    for r in rows:
        d=r["date"]
        yield [r["project"],r["category1"] or "",r["category2"] or "",f"{d[:4]}年",d,f"{d[:7]}",r["member"],f"¥{r["amount_cents"]/100:.2f}"]


@app.get("/api/export/csv")
def export_csv(start: Optional[str]=None,end: Optional[str]=None,_=Depends(require_admin)):
    with get_conn() as conn:
        rows=conn.execute("SELECT * FROM transactions WHERE is_deleted=0 AND (? IS NULL OR date>=?) AND (? IS NULL OR date<=?) ORDER BY date,created_at",(start,start,end,end)).fetchall()
    output=io.StringIO(newline=""); csv.writer(output).writerows(iter_export_rows(rows))
...
def import_csv(body: str = Body(..., media_type="text/csv"), _=Depends(require_admin)):
    with get_conn() as conn:
        count,errors=import_rows(body,conn)
